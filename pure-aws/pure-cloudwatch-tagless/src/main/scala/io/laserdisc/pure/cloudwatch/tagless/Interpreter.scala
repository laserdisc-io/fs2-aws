...
trait Interpreter[M[_]] { outer =>
  ...
  trait CloudWatchAsyncClientInterpreter extends CloudWatchAsyncClientOp[Kleisli[M, CloudWatchAsyncClient, *]] {
    ...
    override def describeAlarmsForMetric(a: DescribeAlarmsForMetricRequest)   = eff(_.describeAlarmsForMetric(a))
    override def describeAlarmsPaginator                                      = primitive(_.describeAlarmsPaginator)
    override def describeAlarmsPaginator(a: DescribeAlarmsRequest)            = primitive(_.describeAlarmsPaginator(a))
    override def describeAnomalyDetectors(a: DescribeAnomalyDetectorsRequest) = eff(_.describeAnomalyDetectors(a))
    ...
    override def getInsightRuleReport(a: GetInsightRuleReportRequest)       = eff(_.getInsightRuleReport(a))
    override def getMetricData(a: GetMetricDataRequest)                     = eff(_.getMetricData(a))
    override def getMetricDataPaginator(a: GetMetricDataRequest)            = primitive(_.getMetricDataPaginator(a))
    ...
    override def getMetricWidgetImage(a: GetMetricWidgetImageRequest)       = eff(_.getMetricWidgetImage(a))
    ...
    override def listDashboardsPaginator(a: ListDashboardsRequest)          = primitive(_.listDashboardsPaginator(a))
    override def listManagedInsightRules(a: ListManagedInsightRulesRequest) = eff(_.listManagedInsightRules(a))
    override def listManagedInsightRulesPaginator(a: ListManagedInsightRulesRequest) = primitive(
      _.listManagedInsightRulesPaginator(a)
    )
    override def listMetricStreams(a: ListMetricStreamsRequest)           = eff(_.listMetricStreams(a))
    override def listMetricStreamsPaginator(a: ListMetricStreamsRequest)  = primitive(_.listMetricStreamsPaginator(a))
    ...
    override def putManagedInsightRules(a: PutManagedInsightRulesRequest) = eff(_.putManagedInsightRules(a))
    ...
    def lens[E](f: E => CloudWatchAsyncClient): CloudWatchAsyncClientOp[Kleisli[M, E, *]] =
      new CloudWatchAsyncClientOp[Kleisli[M, E, *]] {
        ...
        override def describeAnomalyDetectorsPaginator(a: DescribeAnomalyDetectorsRequest) =
        ...
        override def listManagedInsightRules(a: ListManagedInsightRulesRequest) =
          Kleisli(e => eff1(f(e).listManagedInsightRules(a)))
        override def listManagedInsightRulesPaginator(a: ListManagedInsightRulesRequest) =
