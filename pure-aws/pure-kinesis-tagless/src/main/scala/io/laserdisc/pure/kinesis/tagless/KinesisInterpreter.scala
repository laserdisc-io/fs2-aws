...
object KinesisInterpreter {

  def apply[M[_]](
      implicit am: Async[M]
  ): KinesisInterpreter[M] =
    new KinesisInterpreter[M] {
...
trait KinesisInterpreter[M[_]] { outer =>
  ...
  trait KinesisAsyncClientInterpreter extends KinesisAsyncClientOp[Kleisli[M, KinesisAsyncClient, *]] {
    ...
    override def deleteStream(a: DeleteStreamRequest): Kleisli[M, KinesisAsyncClient, DeleteStreamResponse] = eff(
      _.deleteStream(a)
    )
    override def deregisterStreamConsumer(
        a: DeregisterStreamConsumerRequest
    ): Kleisli[M, KinesisAsyncClient, DeregisterStreamConsumerResponse] = eff(_.deregisterStreamConsumer(a))
    ...
    override def describeStream(a: DescribeStreamRequest): Kleisli[M, KinesisAsyncClient, DescribeStreamResponse] = eff(
      _.describeStream(a)
    )
    override def describeStreamConsumer(
        a: DescribeStreamConsumerRequest
    ): Kleisli[M, KinesisAsyncClient, DescribeStreamConsumerResponse] = eff(_.describeStreamConsumer(a))
    override def describeStreamSummary(
        a: DescribeStreamSummaryRequest
    ): Kleisli[M, KinesisAsyncClient, DescribeStreamSummaryResponse] = eff(_.describeStreamSummary(a))
    override def disableEnhancedMonitoring(
        a: DisableEnhancedMonitoringRequest
    ): Kleisli[M, KinesisAsyncClient, DisableEnhancedMonitoringResponse] = eff(_.disableEnhancedMonitoring(a))
    override def enableEnhancedMonitoring(
        a: EnableEnhancedMonitoringRequest
    ): Kleisli[M, KinesisAsyncClient, EnableEnhancedMonitoringResponse] = eff(_.enableEnhancedMonitoring(a))
    override def getRecords(a: GetRecordsRequest): Kleisli[M, KinesisAsyncClient, GetRecordsResponse] = eff(
      _.getRecords(a)
    )
    override def getResourcePolicy(
        a: GetResourcePolicyRequest
    ): Kleisli[M, KinesisAsyncClient, GetResourcePolicyResponse] = eff(_.getResourcePolicy(a))
    override def getShardIterator(
        a: GetShardIteratorRequest
    ): Kleisli[M, KinesisAsyncClient, GetShardIteratorResponse] = eff(_.getShardIterator(a))
    ...
    override def listStreamConsumers(
        a: ListStreamConsumersRequest
    ): Kleisli[M, KinesisAsyncClient, ListStreamConsumersResponse] = eff(_.listStreamConsumers(a))
    override def listStreamConsumersPaginator(
        a: ListStreamConsumersRequest
    ): Kleisli[M, KinesisAsyncClient, ListStreamConsumersPublisher] = primitive(_.listStreamConsumersPaginator(a))
    override def listStreams: Kleisli[M, KinesisAsyncClient, ListStreamsResponse] = eff(_.listStreams)
    override def listStreams(a: ListStreamsRequest): Kleisli[M, KinesisAsyncClient, ListStreamsResponse] = eff(
      _.listStreams(a)
    )
    override def listStreamsPaginator: Kleisli[M, KinesisAsyncClient, ListStreamsPublisher] = primitive(
      _.listStreamsPaginator
    )
    override def listStreamsPaginator(a: ListStreamsRequest): Kleisli[M, KinesisAsyncClient, ListStreamsPublisher] =
      primitive(_.listStreamsPaginator(a))
    override def listTagsForResource(
        a: ListTagsForResourceRequest
    ): Kleisli[M, KinesisAsyncClient, ListTagsForResourceResponse] = eff(_.listTagsForResource(a))
    override def listTagsForStream(
        a: ListTagsForStreamRequest
    ): Kleisli[M, KinesisAsyncClient, ListTagsForStreamResponse] = eff(_.listTagsForStream(a))
    override def mergeShards(a: MergeShardsRequest): Kleisli[M, KinesisAsyncClient, MergeShardsResponse] = eff(
      _.mergeShards(a)
    )
    override def putRecord(a: PutRecordRequest): Kleisli[M, KinesisAsyncClient, PutRecordResponse] = eff(_.putRecord(a))
    override def putRecords(a: PutRecordsRequest): Kleisli[M, KinesisAsyncClient, PutRecordsResponse] = eff(
      _.putRecords(a)
    )
    override def putResourcePolicy(
        a: PutResourcePolicyRequest
    ): Kleisli[M, KinesisAsyncClient, PutResourcePolicyResponse] = eff(_.putResourcePolicy(a))
    override def registerStreamConsumer(
        a: RegisterStreamConsumerRequest
    ): Kleisli[M, KinesisAsyncClient, RegisterStreamConsumerResponse] = eff(_.registerStreamConsumer(a))
    override def removeTagsFromStream(
        a: RemoveTagsFromStreamRequest
    ): Kleisli[M, KinesisAsyncClient, RemoveTagsFromStreamResponse] = eff(_.removeTagsFromStream(a))
    override def serviceClientConfiguration: Kleisli[M, KinesisAsyncClient, KinesisServiceClientConfiguration] =
    ...
    override def splitShard(a: SplitShardRequest): Kleisli[M, KinesisAsyncClient, SplitShardResponse] = eff(
      _.splitShard(a)
    )
    override def startStreamEncryption(
        a: StartStreamEncryptionRequest
    ): Kleisli[M, KinesisAsyncClient, StartStreamEncryptionResponse] = eff(_.startStreamEncryption(a))
    override def stopStreamEncryption(
        a: StopStreamEncryptionRequest
    ): Kleisli[M, KinesisAsyncClient, StopStreamEncryptionResponse] = eff(_.stopStreamEncryption(a))
    override def subscribeToShard(
        a: SubscribeToShardRequest,
        b: SubscribeToShardResponseHandler
    ): Kleisli[M, KinesisAsyncClient, Void] = eff(_.subscribeToShard(a, b))
    override def tagResource(a: TagResourceRequest): Kleisli[M, KinesisAsyncClient, TagResourceResponse] = eff(
      _.tagResource(a)
    )
    override def untagResource(a: UntagResourceRequest): Kleisli[M, KinesisAsyncClient, UntagResourceResponse] = eff(
      _.untagResource(a)
    )
    ...
    override def updateShardCount(
        a: UpdateShardCountRequest
    ): Kleisli[M, KinesisAsyncClient, UpdateShardCountResponse] = eff(_.updateShardCount(a))
    override def updateStreamMode(
        a: UpdateStreamModeRequest
    ...
    def lens[E](f: E => KinesisAsyncClient): KinesisAsyncClientOp[Kleisli[M, E, *]] =
      new KinesisAsyncClientOp[Kleisli[M, E, *]] {
        ...
        override def deleteStream(a: DeleteStreamRequest): Kleisli[M, E, DeleteStreamResponse] =
          Kleisli(e => eff1(f(e).deleteStream(a)))
        override def deregisterStreamConsumer(
            a: DeregisterStreamConsumerRequest
        ): Kleisli[M, E, DeregisterStreamConsumerResponse] = Kleisli(e => eff1(f(e).deregisterStreamConsumer(a)))
        override def describeAccountSettings(
            a: DescribeAccountSettingsRequest
        ...
        override def describeStream(a: DescribeStreamRequest): Kleisli[M, E, DescribeStreamResponse] =
          Kleisli(e => eff1(f(e).describeStream(a)))
        override def describeStreamConsumer(
            a: DescribeStreamConsumerRequest
        ): Kleisli[M, E, DescribeStreamConsumerResponse] = Kleisli(e => eff1(f(e).describeStreamConsumer(a)))
        override def describeStreamSummary(
            a: DescribeStreamSummaryRequest
        ): Kleisli[M, E, DescribeStreamSummaryResponse] = Kleisli(e => eff1(f(e).describeStreamSummary(a)))
        override def disableEnhancedMonitoring(
            a: DisableEnhancedMonitoringRequest
        ): Kleisli[M, E, DisableEnhancedMonitoringResponse] = Kleisli(e => eff1(f(e).disableEnhancedMonitoring(a)))
        override def enableEnhancedMonitoring(
            a: EnableEnhancedMonitoringRequest
        ): Kleisli[M, E, EnableEnhancedMonitoringResponse] = Kleisli(e => eff1(f(e).enableEnhancedMonitoring(a)))
        override def getRecords(a: GetRecordsRequest): Kleisli[M, E, GetRecordsResponse] =
          Kleisli(e => eff1(f(e).getRecords(a)))
        ...
        override def getShardIterator(a: GetShardIteratorRequest): Kleisli[M, E, GetShardIteratorResponse] =
        ...
        override def listStreamConsumersPaginator(
            a: ListStreamConsumersRequest
        ): Kleisli[M, E, ListStreamConsumersPublisher] = Kleisli(e => primitive1(f(e).listStreamConsumersPaginator(a)))
        override def listStreams: Kleisli[M, E, ListStreamsResponse] = Kleisli(e => eff1(f(e).listStreams))
        override def listStreams(a: ListStreamsRequest): Kleisli[M, E, ListStreamsResponse] =
          Kleisli(e => eff1(f(e).listStreams(a)))
        override def listStreamsPaginator: Kleisli[M, E, ListStreamsPublisher] =
          Kleisli(e => primitive1(f(e).listStreamsPaginator))
        override def listStreamsPaginator(a: ListStreamsRequest): Kleisli[M, E, ListStreamsPublisher] =
        ...
        override def listTagsForStream(a: ListTagsForStreamRequest): Kleisli[M, E, ListTagsForStreamResponse] =
          Kleisli(e => eff1(f(e).listTagsForStream(a)))
        override def mergeShards(a: MergeShardsRequest): Kleisli[M, E, MergeShardsResponse] =
          Kleisli(e => eff1(f(e).mergeShards(a)))
        override def putRecord(a: PutRecordRequest): Kleisli[M, E, PutRecordResponse] =
          Kleisli(e => eff1(f(e).putRecord(a)))
        override def putRecords(a: PutRecordsRequest): Kleisli[M, E, PutRecordsResponse] =
          Kleisli(e => eff1(f(e).putRecords(a)))
        ...
        override def registerStreamConsumer(
            a: RegisterStreamConsumerRequest
        ): Kleisli[M, E, RegisterStreamConsumerResponse] = Kleisli(e => eff1(f(e).registerStreamConsumer(a)))
        override def removeTagsFromStream(a: RemoveTagsFromStreamRequest): Kleisli[M, E, RemoveTagsFromStreamResponse] =
          Kleisli(e => eff1(f(e).removeTagsFromStream(a)))
        override def serviceClientConfiguration: Kleisli[M, E, KinesisServiceClientConfiguration] =
        ...
        override def splitShard(a: SplitShardRequest): Kleisli[M, E, SplitShardResponse] =
          Kleisli(e => eff1(f(e).splitShard(a)))
        override def startStreamEncryption(
            a: StartStreamEncryptionRequest
        ): Kleisli[M, E, StartStreamEncryptionResponse] = Kleisli(e => eff1(f(e).startStreamEncryption(a)))
        override def stopStreamEncryption(a: StopStreamEncryptionRequest): Kleisli[M, E, StopStreamEncryptionResponse] =
          Kleisli(e => eff1(f(e).stopStreamEncryption(a)))
        override def subscribeToShard(
            a: SubscribeToShardRequest,
            b: SubscribeToShardResponseHandler
        ): Kleisli[M, E, Void] = Kleisli(e => eff1(f(e).subscribeToShard(a, b)))
        ...
        override def updateStreamMode(a: UpdateStreamModeRequest): Kleisli[M, E, UpdateStreamModeResponse] =
  ...
  def clientResource: Resource[M, KinesisAsyncClient] = clientResource(KinesisAsyncClient.builder())

  @deprecated("use clientResource(builder)", "7.0.0")
  def KinesisAsyncClientResource(builder: KinesisAsyncClientBuilder): Resource[M, KinesisAsyncClient] = clientResource(
    builder
  )

  @deprecated("use resource(builder)", "7.0.0")
  def KinesisAsyncClientOpResource(builder: KinesisAsyncClientBuilder): Resource[M, KinesisAsyncClientOp[M]] = resource(
    builder
  )
  ...
  def create(client: KinesisAsyncClient): KinesisAsyncClientOp[M] = new KinesisAsyncClientOp[M] {
    ...
    override def deregisterStreamConsumer(a: DeregisterStreamConsumerRequest): M[DeregisterStreamConsumerResponse] =
    ...
    override def describeStreamConsumer(a: DescribeStreamConsumerRequest): M[DescribeStreamConsumerResponse] = eff1(
      client.describeStreamConsumer(a)
    )
    override def describeStreamSummary(a: DescribeStreamSummaryRequest): M[DescribeStreamSummaryResponse] = eff1(
      client.describeStreamSummary(a)
    )
    override def disableEnhancedMonitoring(a: DisableEnhancedMonitoringRequest): M[DisableEnhancedMonitoringResponse] =
      eff1(client.disableEnhancedMonitoring(a))
    override def enableEnhancedMonitoring(a: EnableEnhancedMonitoringRequest): M[EnableEnhancedMonitoringResponse] =
    ...
    override def getShardIterator(a: GetShardIteratorRequest): M[GetShardIteratorResponse] = eff1(
      client.getShardIterator(a)
    )
    override def increaseStreamRetentionPeriod(
        a: IncreaseStreamRetentionPeriodRequest
    ...
    override def listStreamConsumersPaginator(a: ListStreamConsumersRequest): M[ListStreamConsumersPublisher] =
    ...
    override def listStreamsPaginator(a: ListStreamsRequest): M[ListStreamsPublisher] = primitive1(
      client.listStreamsPaginator(a)
    )
    ...
    override def listTagsForStream(a: ListTagsForStreamRequest): M[ListTagsForStreamResponse] = eff1(
      client.listTagsForStream(a)
    )
    ...
    override def registerStreamConsumer(a: RegisterStreamConsumerRequest): M[RegisterStreamConsumerResponse] = eff1(
      client.registerStreamConsumer(a)
    )
    override def removeTagsFromStream(a: RemoveTagsFromStreamRequest): M[RemoveTagsFromStreamResponse] = eff1(
      client.removeTagsFromStream(a)
    )
    override def serviceClientConfiguration: M[KinesisServiceClientConfiguration] = primitive1(
    ...
    override def startStreamEncryption(a: StartStreamEncryptionRequest): M[StartStreamEncryptionResponse] = eff1(
      client.startStreamEncryption(a)
    )
    override def stopStreamEncryption(a: StopStreamEncryptionRequest): M[StopStreamEncryptionResponse] = eff1(
      client.stopStreamEncryption(a)
    )
    override def subscribeToShard(a: SubscribeToShardRequest, b: SubscribeToShardResponseHandler): M[Void] = eff1(
    ...
    override def updateShardCount(a: UpdateShardCountRequest): M[UpdateShardCountResponse] = eff1(
      client.updateShardCount(a)
    )
    override def updateStreamMode(a: UpdateStreamModeRequest): M[UpdateStreamModeResponse] = eff1(
      client.updateStreamMode(a)
    )
    override def updateStreamWarmThroughput(
        a: UpdateStreamWarmThroughputRequest
