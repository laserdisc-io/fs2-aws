...
trait Interpreter[M[_]] { outer =>
  ...
  trait SqsAsyncClientInterpreter extends SqsAsyncClientOp[Kleisli[M, SqsAsyncClient, *]] {
    ...
    override def changeMessageVisibility(a: ChangeMessageVisibilityRequest) = eff(
      _.changeMessageVisibility(a)
    )
    ...
    override def listDeadLetterSourceQueues(a: ListDeadLetterSourceQueuesRequest) = eff(
      _.listDeadLetterSourceQueues(a)
    )
  ...
  def create(client: SqsAsyncClient): SqsAsyncClientOp[M] = new SqsAsyncClientOp[M] {
    ...
    override def changeMessageVisibility(a: ChangeMessageVisibilityRequest) = eff1(
    ...
    override def deleteMessageBatch(a: DeleteMessageBatchRequest) = eff1(
      client.deleteMessageBatch(a)
    )
    override def deleteQueue(a: DeleteQueueRequest) = eff1(client.deleteQueue(a))
    override def getQueueAttributes(a: GetQueueAttributesRequest) = eff1(
    ...
    override def listQueuesPaginator(a: ListQueuesRequest) = primitive1(
    ...
    override def setQueueAttributes(a: SetQueueAttributesRequest) = eff1(
